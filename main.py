...
def create_ticket_table(user_list_dict, ticket_list):
...
def create_mapping_ticket_to_user(user, ticket):
    pass
...
def create_index_by_tag(ticket_list, tag_list):
    search_table_by_field = defaultdict(list)
    for ticket in ticket_list:
        try:
            for tag in ticket['tags']:
                search_table_by_field[tag].append(ticket['_id'])
        except KeyError:
            search_table_by_field['missing_field'].append((ticket['_id']))
        except Exception as e:
            print(e)
            raise
    return search_table_by_field
...
def start_search(user_searchable_fields, ticket_searchable_fields):
    ...
    if entity == 'user' and search_term in user_searchable_fields:
        pass
    elif entity == 'ticket' and search_term in ticket_searchable_fields:
        pass
    else:
...
if __name__ == '__main__':
    ...
    # get the user_search_table ready
    user_search_table_list = add_in_ticket_field_to_user_list(user_list, ticket_list)


    user_search_table = convert_list_to_dict(user_list, '_id')
    user_search_table_indexed_by_name = create_index_by_field(user_list, 'name')
    ...
    ticket_search_table_list = create_ticket_table(user_search_table, ticket_list)
    ticket_search_table = convert_list_to_dict(ticket_search_table_list, '_id')
    ...
    # why would I have missing_field here?????
    ...
    ticket_search_table_indexed_by_tag = create_index_by_tag(ticket_list, 'tags')
    ...
    # list_searchable_fields(user_list, "User")
    # list_searchable_fields(ticket_list, "Ticket")
